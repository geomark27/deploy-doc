...
// GetQATasksForReview returns sprint tasks in Testing or En Revisión for the given module.
func (c *Client) GetQATasksForReview(sprintName, module string) ([]QAIssue, error) {
	jql := fmt.Sprintf(
		`project = APP AND sprint = "%s" AND status in ("Testing", "En Revisión") AND component = "%s" ORDER BY key ASC`,
		sprintName, module,
	)
...
func (c *Client) searchQAIssues(jql string) ([]QAIssue, error) {
	path := fmt.Sprintf(
		"/rest/api/3/search?jql=%s&fields=key,summary,customfield_10498,customfield_10134,customfield_10000&maxResults=100",
		url.QueryEscape(jql),
	)
